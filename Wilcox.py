...
fitness_column = {"Baseline": "MH_fitness", "ALNS_AOS": "ALNS_fitness", "ALNS_Q_learning": "ALNS_fitness", "Hybrid_GA_ALNS": "Hybrid_fitness"}

runtime_column = { "Baseline": "MH_runtime", "ALNS_AOS": "ALNS_runtime", "ALNS_Q_learning": "ALNS_runtime", "Hybrid_GA_ALNS": "Hybrid_ALNS_runtime"}
...
def normalize_instance_name(series):
    return (series.astype(str).str.replace("\\", "/", regex=False).str.split("/").str[0])


def normalize_table_name(series):
    return (series.astype(str).str.lower().str.replace(" ", "", regex=False).str.replace("_", "", regex=False))
...
def read_method_file(file_path, method, column_dict):
    ...
    result = result.drop_duplicates(subset=["table", "case", "seed"], keep="first")
...
def build_wide_dataset(column_dict):
    ...
    for method, df in method_data.items():
        if wide is None:
            ...
            wide = wide.merge(df, on=["table", "case", "seed"], how="outer")
    return wide
...
def sort_cases(df):
    ...
    if not table14_df.empty:
        ...
        table14_df["case_sort"] = table14_df["case"].apply(lambda x: order.index(x) if x in order else 999)
...
for table_name in sorted(fitness_wide["table"].dropna().unique()):
    ...
    for case in sorted(table_df["case"].dropna().unique()):
        ...
        row = {"table": table_name, "case": str(case), "N_seeds": case_df["seed"].nunique()}
...
summary_df = case_averages_df.merge(runtime_averages_df, on=["table", "case", "N_seeds"], how="outer")

summary_df = summary_df.merge(paper_results, on=["table", "case"], how="left")

methods_for_arpd = ["Baseline", "ALNS_AOS", "Hybrid_GA_ALNS", "ALNS_Q_learning",]

for method in methods_for_arpd:
    method_col = f"{method}_mean_fitness"

    summary_df[f"{method}_ARPD_vs_Paper_%"] = np.where(summary_df["Paper_result"] == 0, np.nan, ((summary_df[method_col] - summary_df["Paper_result"]) / summary_df["Paper_result"]) * 100)
